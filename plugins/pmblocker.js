...
export async function before(m, { conn, isAdmin, isBotAdmin, isOwner, isROwner }) {
    ...
    if (!m.isGroup) {
      ...
      if (bot.pmblocker && !isOwner && !isROwner) {
        ...
        await m.reply(`*Hey bza @${m.sender.split`@`[0]}, messaging the —(••÷[ Š𝓊ḅᤂ𝖗𝓊 ]÷••)—─❂  bot privately is currently disabled. You have been blocked from using the —(••÷[ Š𝓊ḅᤂ𝖗𝓊 ]÷••)—─❂ bot.*`, false, { mentions: [m.sender] });
